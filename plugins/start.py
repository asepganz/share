...
async def start_command(client: Bot, message: Message):
    ...
    if len(text) > 7:
        ...
        # Panggil biasa tanpa 'await' karena di button.py menggunakan 'def' biasa
...
async def not_joined(client: Bot, message: Message):
    # Panggil biasa tanpa 'await' agar sinkron dengan button.py yang tidak menyembunyikan tombol
...
async def ping_pong(client, m: Message):
    ...
    await m_reply.edit_text(    if seconds == 0:
        return "inf"
    parts = []
    for unit, div in TIME_DURATION_UNITS:
        amount, seconds = divmod(int(seconds), div)
        ...
        try:
            ...
            for msg in messages:
                ...
                try:
                    ...
                    await asyncio.sleep(e.x)
                    await msg.copy(chat_id=id, caption=caption, reply_markup=reply_markup)
                except Exception:
                    pass
        except Exception:
            await message.reply_text("<b>Telah Terjadi Error 🥺</b>")
    else:
        # PENGGUNAAN AWAIT PADA START_BUTTON
...
@Bot.on_message(filters.command("start") & filters.private)
async def not_joined(client: Bot, message: Message):
    # PENGGUNAAN AWAIT PADA FSUB_BUTTON
...
async def send_text(client: Bot, message: Message):
    if not message.reply_to_message:
        return await message.reply("Gunakan perintah ini dengan me-reply pesan.")
    
    query = await query_msg()
    broadcast_msg = message.reply_to_message
    successful, blocked, deleted, unsuccessful = 0, 0, 0, 0
    
    pls_wait = await message.reply("<code>Broadcasting...</code>")
    for row in query:
        chat_id = int(row[0])
        try:
            await broadcast_msg.copy(chat_id, protect_content=PROTECT_CONTENT)
